/**
 * React component for rendering a form template with customer and order information.
 * @param {Object} props - The props object.
 * @param {Object} props.data - The data object containing the order details.
 * @param {string} props.orderName - The name of the order.
 * @param {string} props.orderFor - The name of the person/company the order is for.
 * @param {string} props.collectionOf - The name of the collection in the data object.
 * @param {string} props.date - The name of the date field in the data object.
 * @param {string} props.formWidth - The width of the form (default: "w-full").
 * @returns {JSX.Element} The JSX element representing the form template.
 */
...
const FormTemplate = ({
  ...
  return (
    <>
      ...
      />
      <main className={`${[formWidth]} mx-auto p-5`} ref={componentRef}>
        <CompanyInfo />
        <section className="w-full">
          <h1 className="text-4xl font-bold text-center uppercase underline">
            {orderName}
          </h1>
          <h4 className="text-xl font-bold text-center uppercase underline">
            {data?.currency?.name}
          </h4>
        </section>
        ...
        <section className="flex justify-between w-full my-2 border-1 rounded-md divide-x-1">
          <div className="flex-1">
            ...
            <div className="my-2 p-2">
              <CustomerInfo customerData={data[orderFor]} infoOf={orderFor} />
            </div>
          </div>
          <div className="flex-1">
            <h6 className="text-md text-darkfs font-semibold bg-gray-100 p-1 pl-2">
        ...
        <section className="w-full my-2 flex flex-col items-end">
          ...
          <article className="w-1/4 border-1 font-semibold rounded-md align-middle divide-y-1">
            ...
            <aside className="flex items-center justify-between divide-x-1">
              ...
              <h6 className="flex-1 p-1 text-sm">
                {total.total_without_vat} /=
              </h6>
            </aside>
            <aside className="flex items-center justify-between divide-x-1">
